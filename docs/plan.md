...
- **Haberleşme:** Kestrel üzerinde düz WebSocket ve JSON mesajlar, port 9820.
  - Server → client: `welcome` (+ pairing token), `layout.full`, `layout.patch` (a saved edit reaches a client as only the changed widgets; the client announces `layout.patch` in `hello.capabilities`, see `the server repo's docs/done/layout-patch-and-assets.md`), `widget.state` (render edilmiş text, değer, stil override'ı), `profiles.list`, `asset` (large `data:` values such as icons are sent once per client as `asset:<hash>` references and cached there)
...
5. **Client (Aşama 5 tamam).** Eşleştirme — PIN ve QR tamam, mDNS henüz yok. Cihaz bazında profil ataması server/editör tarafında tamam (bu repoda ek iş gerekmedi, drawer server'ın atadığı profili açıyor). Profil drawer'ı tamam. Sayfa geçişi client'tan tetikleniyor (yatay swipe → `page.next`/`page.prev`). Keep-awake + yeniden bağlanma + son-layout önbelleği tamam. Kiosk modu (immersive) ve yön kilidi tamam (`KioskPlugin.java`, `@capacitor/screen-orientation`), drawer içindeki Ayarlar panelinden açılıp kapanıyor. Tarayıcı client server repo'sunda (`webclient/`, `/deck/`) tamamlandı. **Gerçek cihazda USB debug testi de yapıldı (2026-09-23):** bir Android telefona `adb install` ile debug APK kuruldu, önceden eşleştirilmiş cihaz sunucuya gerçek LAN üzerinden yeniden bağlandı, canlı değişken push'ı (ses seviyesi widget'ı yüzdesi/rengi gerçek zamanlı değişti), dokunma girişi ve tek sayfalı profilde swipe'ın no-op kalması doğrulandı.
...
- Uçtan uca test: 4x3 grid'de 2x2 bir buton, bir ses slider'ı ve bir sohbet web widget'ı oluşturulur, ayrıca ikinci bir sayfa ve ona geçiş butonu eklenir. İki cihaz (emülatör + tarayıcı) eşleştirilir ve her birine farklı profil atanır. Kontrol edilecekler:
  - Butonla bir metin editöründe `ctrl+v` çalışıyor
