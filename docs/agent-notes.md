...
- **QR ile eşleştirme tamam:** `QrScan.tsx` (`@capacitor-mlkit/barcode-scanning`) editördeki `PairingWindow`'un ürettiği `macrostation://pair?host=…&port=…&pin=…` (ya da düz `ip:port:pin`/`ip:port`) kodunu tarayıp ayrıştırıyor, host bulunamazsa/kod tanınmazsa hata gösteriyor, bulununca "bu sunucuya bağlanılsın mı?" onayı sonrası `ConnectScreen`'e host+PIN'i geçiriyor. Gerçek sunucuya karşı uçtan uca doğrulandı.
...
- **Henüz yok:** mDNS otomatik keşif (native Android NSD köprüsü gerektiren bir Capacitor plugin'i yazmak gerekiyor — ayrı, büyük bir iş), kiosk modu, gerçek cihaz/emülatör testi (şimdiye kadar sadece tarayıcıda `npm run dev` ile test edildi), slider/knob'un gerçek çift yönlü değer akışı.
